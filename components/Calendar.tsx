...
const Calendar = () => {
            ...
            {week.map((day, index) => {
              if (day !== null) {
                return (
                  <TouchableOpacity
                    key={`${month}-${day}-${index}`}
                    onPress={() => day && handleDayPress(day, month, index)}
                    style={
                ...
                return <View key={index} style={styles.dayBlank} />;
              }
            })}
          </View>
        ))}
   ...
   */
  const handleDayPress = (day: number, month: number, index: number) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text style={{ color: "white", fontSize: 22 }}>{selectedYear}</Text>
        <Pressable onPress={() => setSelectedYear((prevYear) => prevYear + 1)}>
          {({ pressed }) => (
            <Entypo
      ...
      <ScrollView>
        {months.map((monthName, index) => {
          return (
            <View key={months[index]} style={styles.monthContainer}>
              <Text
